...
@interface AAGroupViewController () {
...
@implementation AAGroupViewController
...
- (void)refreshAssetsGroups:(BOOL)force {
    NSLog(@"AAGroupViewController refreshAssetsGroups");
    AAAssetLibHelper *assetLibHelper = [AAAssetLibHelper defaultAssetLibHelper];
...
- (void)selectGroup:(NSString *)groupPersistentID {
    if (groupPersistentID) {
        AAItemViewController *itemViewController = [[[AAItemViewController alloc] initWithStyle:UITableViewStylePlain] autorelease];
        itemViewController.groupPersistentID = groupPersistentID;
        
        [self.navigationController pushViewController:itemViewController animated:YES];
    } else {
        [NSException raise:@"AAGroupViewController error" format:@"Reason: groupPersistentID == nil"];
    }
}
...
- (NSUInteger)calcFrameSize {
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
        ...
        [NSException raise:@"AAGroupViewController error" format:@"Reason: Current device type unknown"];
        return 0;
    }
}
...
- (NSUInteger)calcItemCountInCell {
    if (_frameSize == 0) {
        [NSException raise:@"AAGroupViewController error" format:@"Reason: frameSize == 0"];
    }
...
- (NSUInteger)calcCellSpace {
    ...
    if (_itemCountInCell == 0) {
        [NSException raise:@"AAGroupViewController error" format:@"Reason: itemCountInCell == 0"];
    }
    CGRect bounds = [self.tableView bounds];
...
- (void)viewDidLoad
{
    NSLog(@"AAGroupViewController viewDidLoad:");
    [super viewDidLoad];
...
- (void)viewWillAppear:(BOOL)animated {
    NSLog(@"AAGroupViewController viewWillAppear:");
...
- (void)viewWillDisappear:(BOOL)animated {
    NSLog(@"AAGroupViewController viewWillDisappear:");
...
- (void)viewDidUnload
{
    NSLog(@"AAGroupViewController viewDidUnload:");
    /*** *** ***/
    NSNotificationCenter *notificationCenter = [NSNotificationCenter defaultCenter];
...
{
    NSLog(@"AAGroupViewController shouldAutorotateToInterfaceOrientation:");
    BOOL result = NO;
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    NSLog(@"AAGroupViewController numberOfSectionsInTableView:");
    return 1;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    NSLog(@"AAGroupViewController tableView:numberOfRowsInSection:");
...
{
    NSLog(@"AAGroupViewController tableView:cellForRowAtIndexPath: indexPath.row = %d", [indexPath row]);
    NSArray *groupPersistentIDs = [self getGroupPersistentIDsForCellAtIndexPath:indexPath];
    AAGroupViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"AAGroupViewCell"];
    if (cell == nil) {
        cell = [[[AAGroupViewCell alloc] initWithGroupPersistentIDs:groupPersistentIDs cellSpace:_cellSpace cellTopBottomMargin:(_cellSpace / 2) frameSize:_frameSize andItemCount:_itemCountInCell] autorelease];
    } else {
