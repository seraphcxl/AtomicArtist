...
- (NSUInteger)getIndexForAssetURL:(NSURL *)assetURL {
    if (assetURL) {
        ...
        if (find) {
            ...
            [NSException raise:@"AAAssetsGroup Error" format:@"Reason: assetURL: %@ not find", assetURL.absoluteString];
            return 0;
        }
    } else {
        [NSException raise:@"AAAssetsGroup Error" format:@"Reason: assetURL is nil"];
        return 0;
    }
}
...
- (NSURL *)getAssetURLAtIndex:(NSUInteger)index {
    if (_allAssetURLs) {
        if (index < [_allAssetURLs count]) {
            ...
            [NSException raise:@"AAAssetsGroup Error" format:@"Reason: Index: %d >= allAssetURLs count: %d", index, [_allAssetURLs count]];
            return nil;
        }
    } else {
        [NSException raise:@"AAAssetsGroup Error" format:@"Reason: allAssetURLStrs is nil"];
        return nil;
    }
}
...
//                        [allAssetURLs addObject:url];
            ...
            [NSException raise:@"AAAssetsGroup Error" format:@"Reason: allAssets is nil"];
        }
    } else {
        [NSException raise:@"AAAssetsGroup Error" format:@"Reason: self.assetsGroup is nil"];
    }
