...
static AAAssetLibHelper *staticDefaultAssetLibHelper = nil;

@interface AAAssetLibHelper () {
    ALAssetsLibrary *_assetLib;
    NSMutableArray *_allALGroupPersistentIDs;
    NSMutableDictionary *_allALGroups; // Key:(NSSString *)groupPersistentID Value:(AAAssetsGroup *)assetsGroup
}

@end

@implementation AAAssetLibHelper

- (NSUInteger)getIndexInGoupPersistentID:(NSString *)groupPersistentID forAssetURL:(NSURL *)assetURL {
    if (_allALGroups) {
        AAAssetsGroup *assetsGroup = [_allALGroups objectForKey:groupPersistentID];
        if (assetsGroup) {
            return [assetsGroup getIndexForAssetURL:assetURL];
        } else {
//            [NSException raise:@"AAAssetLibHelper error" format:@"Reason: assetsGroup for %@ not find", groupPersistentID];
            NSLog(@"Reason: assetsGroup for %@ not find", groupPersistentID);
            return 0;
        }
    } else {
        [NSException raise:@"AAAssetLibHelper error" format:@"Reason: allALGroups is nil"];
        return 0;
    }
}
...
- (NSString *)getGoupPersistentIDAtIndex:(NSUInteger)index {
    if (_allALGroupPersistentIDs) {
        if (index < [_allALGroupPersistentIDs count]) {
            ...
            [NSException raise:@"AAAssetLibHelper Error" format:@"Reason: Index: %d >= allALGroupPersistentIDs count: %d", index, [_allALGroupPersistentIDs count]];
            return nil;
        }
    } else {
        [NSException raise:@"AAAssetLibHelper Error" format:@"Reason: allALGroupPersistentIDs is nil"];
        return nil;
    }
}
...
- (BOOL)enumerateGroupsWithTypes:(ALAssetsGroupType)types {
    ...
    void (^ALAssetsLibraryGroupsEnumerationResultsBlock)(ALAssetsGroup *group, BOOL *stop) = ^(ALAssetsGroup *group, BOOL *stop) {
		if (group != nil) {
            ...
            if (result == nil) {
                AAAssetsGroup *assetsGroup = [[[AAAssetsGroup alloc] init] autorelease];
...
//                [allALGroupPersistentIDs addObject:groupPersistentID];
    ...
    if (_assetLib) {
        ...
        [NSException raise:@"AAAssetLibHelper error" format:@"Reason: assetLib is nil"];
    }
...
- (ALAsset *)getALAssetForGoupPersistentID:(NSString *)groupPersistentID forAssetURL:(NSURL *)assetURL {
    ...
    if (_allALGroups) {
        ...
        if (assetsGroup) {
            ...
            [NSException raise:@"AAAssetLibHelper error" format:@"Reason: assetsGroup for %@ is nil", groupPersistentID]; 
        }
    } else {
        [NSException raise:@"AAAssetLibHelper error" format:@"Reason: allALGroups is nil"];
    }
